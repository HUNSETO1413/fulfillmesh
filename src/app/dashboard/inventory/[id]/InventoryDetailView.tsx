...
const RANGE_OPTIONS = ["7 Days", "30 Days", "90 Days"];
...
export default function InventoryDetailView({ item }: { item: InventoryItem }) {
  ...
  const [movementRange, setMovementRange] = useState("30 Days");
  ...
  async function createPurchaseOrder() {
    ...
    try {
      ...
      toast(`Purchase order drafted for ${item.sku} (4,000 units)`);
      setPoCreated(true);
      router.refresh();
    } catch (e) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => toast("Date range filtering is coming soon")} className="flex items-center gap-2 px-3.5 py-2 bg-white border border-[#E2E8F0] rounded-lg text-[13px] font-medium text-[#64748B] shadow-[0_1px_2px_rgba(0,0,0,0.05)] hover:bg-[#F8FAFC]">
          ...
          <button onClick={() => toast("No additional actions for this SKU")} aria-label="More options" className="w-9 h-9 flex items-center justify-center text-[#64748B] bg-white border border-[#E2E8F0] rounded-lg shadow-[0_1px_2px_rgba(0,0,0,0.05)] hover:bg-[#F8FAFC]"><MoreHorizontal className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: "1.1fr 1.3fr 1fr" }}>
        ...
        <div className={`${card} p-4`}>
          ...
          <button onClick={createPurchaseOrder} disabled={busy || poCreated} className="w-full mt-4 text-[13px] font-medium text-white bg-[#3B82F6] hover:bg-[#2563EB] rounded-lg py-2 disabled:opacity-60">{poCreated ? "Purchase Order Created" : "Create Purchase Order"}</button>
        </div>
      </div>
