...
const tabs = [
  { label: "All Invoices", count: 39 },
  { label: "Outstanding", count: 12 },
  { label: "Overdue", count: 5 },
  { label: "Paid", count: 32 },
  { label: "Draft", count: 10 },
  { label: "Cancelled", count: 0 },
];

type Status = "Due Soon" | "Overdue" | "Paid" | "Draft";
...
const invoices: {
  ...
  { num: "INV-2025-0543", po: "SO-100976", date: "May 29, 2025", due: "Jun 12, 2025", amount: "$4,256.75", status: "Due Soon" },
  { num: "INV-2025-0542", po: "SO-100345", date: "May 28, 2025", due: "Jun 11, 2025", amount: "$1,892.00", status: "Due Soon" },
  { num: "INV-2025-0541", po: "SO-100333", date: "May 27, 2025", due: "Jun 9, 2025", amount: "$3,709.00", status: "Due Soon" },
  { num: "INV-2025-0540", po: "PO-50673", date: "May 25, 2025", due: "Jun 8, 2025", amount: "$6,420.00", status: "Overdue" },
  { num: "INV-2025-0539", po: "SO-50671", date: "May 24, 2025", due: "Jun 1, 2025", amount: "$3,140.00", status: "Overdue" },
  { num: "INV-2025-0538", po: "SO-100765", date: "May 23, 2025", due: "May 30, 2025", amount: "$2,940.00", status: "Paid" },
  { num: "INV-2025-0537", po: "SO-100761", date: "May 22, 2025", due: "May 29, 2025", amount: "$4,310.00", status: "Paid" },
  { num: "INV-2025-0536", po: "SO-100748", date: "May 21, 2025", due: "May 28, 2025", amount: "$1,560.00", status: "Paid" },
  { num: "INV-2025-0535", po: "SO-100749", date: "May 20, 2025", due: "May 27, 2025", amount: "$5,942.50", status: "Paid" },
  { num: "INV-2025-0534", po: "SO-100150", date: "May 19, 2025", due: "May 27, 2025", amount: "$3,250.75", status: "Paid" },
];
...
function StatusBadge({ s }: { s: Status }) {
  const styles: Record<Status, string> = {
    "Due Soon": "bg-[#F59E0B]/10 text-[#F59E0B]",
    Overdue: "bg-[#EF4444]/10 text-[#EF4444]",
    Paid: "bg-[#10B981]/10 text-[#10B981]",
    Draft: "bg-[#64748B]/10 text-[#64748B]",
  };
  return <span className={`inline-flex px-2.5 py-0.5 text-[12px] font-medium rounded-md ${styles[s]}`}>{s}</span>;
}
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex items-center gap-1 px-4 border-b border-[#E2E8F0] overflow-x-auto">
            {tabs.map((t) => (
              ...
              >
                {t.label}
                <span className={`text-[11px] px-1.5 py-0.5 rounded-full ${active === t.label ? "bg-[#3B82F6]/10 text-[#3B82F6]" : "bg-[#F1F5F9] text-[#94A3B8]"}`}>{t.count}</span>
              </button>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.num} className="border-b border-[#E2E8F0] last:border-b-0 hover:bg-[#F8FAFC]/60 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#EF4444] shrink-0" />
                        <span className="text-[13px] font-medium text-[#3B82F6] font-mono">{inv.num}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[13px] text-[#64748B] font-mono">{inv.po}</td>
                    <td className="px-4 py-3 text-[13px] text-[#64748B]">{inv.date}</td>
                    <td className="px-4 py-3 text-[13px] text-[#64748B]">{inv.due}</td>
                    <td className="px-4 py-3 text-[13px] font-semibold text-[#1E293B]">{inv.amount}</td>
                    <td className="px-4 py-3"><StatusBadge s={inv.status} /></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        {(inv.status === "Overdue" || inv.status === "Due Soon") && (
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-[#E2E8F0]">
            <p className="text-[12px] text-[#64748B]">Showing 1 to 10 of 39 invoices</p>
            <div className="flex items-center gap-1">
              <button className="w-7 h-7 flex items-center justify-center rounded-md border border-[#E2E8F0] text-[#94A3B8]"><ChevronLeft className="w-4 h-4" /></button>
              <button className="w-7 h-7 flex items-center justify-center rounded-md bg-[#3B82F6] text-white text-[12px] font-medium">1</button>
              <button className="w-7 h-7 flex items-center justify-center rounded-md border border-[#E2E8F0] text-[#64748B] text-[12px]">2</button>
              <button className="w-7 h-7 flex items-center justify-center rounded-md border border-[#E2E8F0] text-[#64748B] text-[12px]">3</button>
              <button className="w-7 h-7 flex items-center justify-center rounded-md border border-[#E2E8F0] text-[#64748B]"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
                  ...
                  return byStatus.map((b, i) => {
                    ...
                    const el = (
                      <circle
                        ...
                        strokeWidth="11"
                        strokeDasharray={`${len} ${circumference - len}`}
                        strokeDashoffset={-(off2 / 100) * circumference}
                      />
                    );
                    off2 += b.pct;
                    return el;
