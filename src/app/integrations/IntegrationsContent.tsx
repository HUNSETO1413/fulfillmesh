...
import { ArrowRight, Search, Check, Plug, ChevronDown, ShieldCheck, Loader2 } from "lucide-react";
...
type Category = "E-commerce" | "Marketplaces" | "Shipping" | "CRM & Marketing" | "Other";
...
interface Integration {
  ...
  connected: boolean;
  category: Category;
  popularity: number; // higher = more popular, used for "Popular" sort
}

const integrations: Integration[] = [
  { name: "Shopify", desc: "Sync orders, inventory, and tracking in real time.", connected: true, category: "E-commerce", popularity: 100 },
  { name: "WooCommerce", desc: "Automate fulfillment and keep customers updated.", connected: false, category: "E-commerce", popularity: 88 },
  { name: "Amazon", desc: "Seamlessly manage orders and inventory.", connected: false, category: "Marketplaces", popularity: 95 },
  { name: "Salesforce", desc: "Keep customer and order data in sync.", connected: false, category: "CRM & Marketing", popularity: 80 },
  { name: "BigCommerce", desc: "Connect your store and automate fulfillment.", connected: false, category: "E-commerce", popularity: 70 },
  { name: "eBay", desc: "Streamline order sync and shipment updates.", connected: false, category: "Marketplaces", popularity: 72 },
  { name: "Etsy", desc: "Simplify order management and fulfillment.", connected: false, category: "Marketplaces", popularity: 65 },
  { name: "Walmart", desc: "Integrate and fulfill orders from Walmart.", connected: false, category: "Marketplaces", popularity: 68 },
  { name: "ShipStation", desc: "Import orders and compare shipping rates.", connected: false, category: "Shipping", popularity: 78 },
  { name: "EasyPost", desc: "Access powerful shipping APIs and tools.", connected: false, category: "Shipping", popularity: 60 },
  { name: "Slack", desc: "Get real-time notifications and updates.", connected: true, category: "Other", popularity: 85 },
  { name: "Zapier", desc: "Automate workflows between FulfillMesh and 5,000+ apps.", connected: false, category: "Other", popularity: 82 },
  { name: "HubSpot", desc: "Sync contacts and order activity to your CRM.", connected: false, category: "CRM & Marketing", popularity: 74 },
];

const FEATURED_NAMES = ["Shopify", "WooCommerce", "Amazon", "Salesforce"];

const filterCategories: { label: string; value: Category | "All" }[] = [
  { label: "All Integrations", value: "All" },
  { label: "E-commerce", value: "E-commerce" },
  { label: "Marketplaces", value: "Marketplaces" },
  { label: "Shipping", value: "Shipping" },
  { label: "CRM & Marketing", value: "CRM & Marketing" },
  { label: "Other", value: "Other" },
];

type SortKey = "popular" | "name-asc" | "name-desc" | "connected";
    ...
    <div className="flex flex-col rounded-xl border border-[#E2E8F0] bg-white p-6 transition-shadow hover:shadow-md">
      ...
      {item.connected ? (
        ...
        <button
          ...
          className="mt-4 w-full rounded-md border border-[#3b82f6] bg-white py-2 text-sm font-semibold text-[#3b82f6] transition-colors hover:bg-[#eff6ff]"
        >
          Connect
        </button>
      )}
...
export default function IntegrationsContent() {
  ...
  // Counts per category reflect the full catalog (stable badge numbers).
  ...
  const withState = useMemo(
    () => integrations.map((i) => ({ ...i, connected: connected[i.name] ?? false })),
    [connected],
  );
  ...
  const sortFn = useMemo(() => {
    return (a: Integration, b: Integration) => {
      switch (sortKey) {
        ...
        case "connected":
          return Number(b.connected) - Number(a.connected) || b.popularity - a.popularity;
        case "popular":
        default:
  ...
  const featured = useMemo(
    () => withState.filter((i) => FEATURED_NAMES.includes(i.name)).sort((a, b) => FEATURED_NAMES.indexOf(a.name) - FEATURED_NAMES.indexOf(b.name)),
    [withState],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return withState
      ...
      .slice()
      .sort(sortFn);
  }, [withState, category, query, sortFn]);

  const activeSortLabel = sortOptions.find((s) => s.key === sortKey)?.label ?? "Popular";

  const confirmConnect = async () => {
    if (!connectTarget) return;
    setConnecting(true);
    // Simulate the OAuth handshake completing.
    await new Promise((r) => setTimeout(r, 700));
    setConnected((prev) => ({ ...prev, [connectTarget.name]: true }));
    setConnecting(false);
  ...
  return (
    <>
                ...
                >
                  ...
                  <span
                    className={`inline-flex min-w-[20px] items-center justify-center rounded-full px-1.5 py-0.5 text-[11px] font-semibold ${
                      active ? "bg-[#1e3a8a] text-white" : "bg-[#e2e8f0] text-[#475569]"
                    }`}
                  >
                    {counts[t.value] ?? 0}
                  </span>
      ...
      {/* Featured Integrations — hidden while searching/filtering for focus */}
        ...
        <section className="bg-white">
          <div className="max-w-[1200px] mx-auto px-8 pt-12 pb-6">
            ...
            <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {featured.map((item) => (
                <IntegrationCard key={item.name} item={item} onConnect={setConnectTarget} />
              ))}
            </div>
          </div>
        </section>
      ...
      <section className="bg-white">
        <div className="max-w-[1200px] mx-auto px-8 pt-8 pb-16">
          <div className="flex items-center justify-between">
            ...
            <div className="relative">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-md border border-[#e2e8f0] px-3 py-2 text-sm text-[#475569] hover:border-[#cbd5e1] transition-colors"
              >
                Sort by: <span className="font-medium text-[#1e293b]">{activeSortLabel}</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform ${sortOpen ? "rotate-180" : ""}`} />
              </button>
              {sortOpen && (
                <ul
                  role="listbox"
                  className="absolute right-0 top-full mt-1 w-48 rounded-md border border-[#e2e8f0] bg-white py-1 shadow-lg z-10"
                >
                  {sortOptions.map((opt) => (
                    <li key={opt.key}>
                      <button
                        ...
                        className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm transition-colors hover:bg-[#f1f5f9] ${
                          opt.key === sortKey ? "font-semibold text-[#1e3a8a]" : "text-[#475569]"
                        }`}
                      >
                        {opt.label}
                        {opt.key === sortKey && <Check className="w-3.5 h-3.5" strokeWidth={3} />}
                      </button>
                    </li>
                  ))}
                </ul>
          ...
          {filtered.length === 0 ? (
            <div className="mt-10 rounded-xl border border-dashed border-[#e2e8f0] bg-[#f8fafc] py-16 text-center">
              <p className="text-sm font-medium text-[#475569]">No integrations match your search.</p>
              <button
                type="button"
                onClick={() => { setQuery(""); setCategory("All"); }}
                className="mt-3 text-sm font-semibold text-[#3b82f6] hover:underline"
              >
                Clear filters
              </button>
            </div>
      ...
      <Modal
        ...
        onClose={() => { if (!connecting) setConnectTarget(null); }}
        title={connectTarget ? `Connect ${connectTarget.name}` : "Connect"}
        description="Authorize FulfillMesh to sync with this tool."
        size="sm"
        footer={
          <>
            <button
              ...
              disabled={connecting}
              className="rounded-md border border-[#e2e8f0] px-4 py-2 text-sm font-semibold text-[#475569] hover:bg-[#f1f5f9] transition-colors disabled:opacity-60"
            >
              Cancel
            </button>
            <button
              ...
              disabled={connecting}
              className="inline-flex items-center gap-2 rounded-md bg-[#3b82f6] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2563eb] transition-colors disabled:opacity-60"
            >
        ...
        {connectTarget && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <LogoMark name={connectTarget.name} />
              <div>
                <p className="text-sm font-semibold text-[#1e293b]">{connectTarget.name}</p>
                <p className="text-xs text-[#64748b]">{connectTarget.category}</p>
              </div>
            </div>
            <p className="text-sm text-[#475569] leading-relaxed">
              You&apos;ll be redirected to {connectTarget.name} to sign in and grant access.
              Once authorized, FulfillMesh will securely sync orders, inventory, and tracking
              for {connectTarget.name}.
            </p>
            <div className="flex items-start gap-2 rounded-lg bg-[#f1f5f9] px-3 py-2.5 text-xs text-[#475569]">
              <ShieldCheck className="w-4 h-4 shrink-0 text-[#059669] mt-px" />
              <span>We use OAuth 2.0 — your credentials are never shared with FulfillMesh.</span>
            </div>
          </div>
        )}
      </Modal>
